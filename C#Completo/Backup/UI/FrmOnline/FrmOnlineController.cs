...
using System.IO;      // Para escrever arquivos de texto
...
{     
    public class FrmOnlineController
    {
        ...
        public void AdicionarInner()
        {
            ...
            objInner.CartMaster = UpdateDisplay.HabCartaoMaster;
            objInner.Master = UpdateDisplay.CartaoMaster;
        ...
        /// <summary>
        /// Método responsável pela liberação de acesso.
        /// CORRIGIDO: Agora libera qualquer ID que a catraca reconhecer.
        /// </summary>
        /// <param name="NumCartao">O ID do usuário vindo da catraca (rosto ou cartão)</param>
        /// <returns></returns>
        private bool LiberarAcesso(string NumCartao)
        {
            // Se o método foi chamado, é porque a catraca JÁ RECONHECEU 
            // um rosto/cartão. Vamos confiar nela e o acesso.

            // Escreva no console de Debug qual ID foi lido (bom para testes)
            System.Diagnostics.Debug.WriteLine("Acesso liberado para o ID: " + NumCartao);

            return true; // <-- Sempre retorna "Verdadeiro" (Acesso Liberado)
        }
        ...
        private void MontaConfiguracaoInner(Inner innerAtual, Enumeradores.modoComunicacao modo)
        {
            ...
            //Para QRCode deve utilizar dígitos variáveis
            if (innerAtual.TipoLeitor == (byte)Enumeradores.TipoLeitor.QRCODE_LETRAS)
            {
        ...
        private static void ConfigurarTpLeitor(Inner innerAtual)
        {
            switch (innerAtual.TipoLeitor)
            {
                ...
                case (byte)Enumeradores.TipoLeitor.CODIGO_BARRAS_SERIAL:
                    ...
                    break;
                case (byte)Enumeradores.TipoLeitor.WIEGAND_FC_SEM_SEPARADOR:
                    EasyInner.ConfigurarTipoLeitor((byte)Enumeradores.TipoLeitor.WIEGAND_FC_SEM_SEPARADOR);
                    break;
                case (byte)Enumeradores.TipoLeitor.WIEGAND_FC_COM_SEPARADOR:
                    EasyInner.ConfigurarTipoLeitor((byte)Enumeradores.TipoLeitor.WIEGAND);
                    break;
                case (byte)Enumeradores.TipoLeitor.QRCODE_LETRAS:
                    EasyInner.ConfigurarTipoLeitor((byte)Enumeradores.TipoLeitor.BARRAS_PROX_QRCODE);
                    break;
                default:
                    EasyInner.ConfigurarTipoLeitor((byte)Enumeradores.TipoLeitor.CODIGO_DE_BARRAS);
                    break;
        ...
        private void MontarListaLivre(Inner InnerAtual, List<Usuarios> Lista)
        {
            ...
            // Se a lista veio nula (porque o .mdb não foi pedido), 
            // simplesmente pare de executar este método e não quebre.
            if (Lista == null)
            {
                return;
            }
        ...
        private void MontarBilheteRecebido(Inner InnerAtual)
        {
            ...
            ListaParaJSON.Add(InnerAtual.BilheteOnline);
        ...
        /// <summary>
        /// AÇÃO 1: Envia o evento para a API da sua equipe em tempo real.
        /// </summary>
        private void EnviarEventoParaAPI(Bilhete bilhete)
        {
            // SUBSTITUIR PELA URL DA API A SE ENVIAR
            string urlDaAPI = "URL NA QUAL VAI SER ENVIADA";

            try
            {
                // Formata o bilhete em JSON
                ...
                string jsonFinal = $"{{ \"Cartao\": \"{cartaoLimpo}\", \"Data\": \"{dataFormatada}\", \"Hora\": \"{horaFormatada}\", \"Origem\": {bilhete.Origem}, \"Tipo\": {bilhete.Tipo} }}";

                // Envia para a API (Não trava o programa)
                using (WebClient client = new WebClient())
                {
                    client.Headers[HttpRequestHeader.ContentType] = "application/json";
                    client.UploadStringAsync(new Uri(urlDaAPI), "POST", jsonFinal);
                }
                Debug.WriteLine("--> Evento enviado para API: " + jsonFinal);
            }
            catch (Exception ex)
            {
                Debug.WriteLine("!!! ERRO AO ENVIAR PARA API: " + ex.Message);
            }
        }

        /// <summary>
        /// AÇÃO 2: Salva o evento em um arquivo de log local (.txt)
        /// </summary>
        private void SalvarEventoLocalmente(Bilhete bilhete)
        {
            // Define o arquivo de log (ex: C:\Logs\Catraca_Log_2025-11-11.txt)
            string caminhoDaPasta = @"C:\Logs\";
            string nomeArquivo = $"Catraca_Log_{DateTime.Now.ToString("yyyy-MM-dd")}.txt";
            string caminhoCompleto = Path.Combine(caminhoDaPasta, nomeArquivo);

            try
            {
                // Garante que a pasta C:\Logs exista
                Directory.CreateDirectory(caminhoDaPasta);

                // Formata o bilhete para uma linha de texto (CSV)
                string dataFormatada = $"{bilhete.Dia:D2}/{bilhete.Mes:D2}/20{bilhete.Ano:D2}";
                string horaFormatada = $"{bilhete.Hora:D2}:{bilhete.Minuto:D2}:{bilhete.Segundo:D2}";
                string cartaoLimpo = bilhete.Cartao.ToString().Trim();

                string linhaLog = $"{dataFormatada};{horaFormatada};{cartaoLimpo};{bilhete.Origem};{bilhete.Tipo}";

                // O 'true' significa "Append" (adicionar ao final do arquivo, sem apagar)
                using (StreamWriter sw = new StreamWriter(caminhoCompleto, true))
                {
                    sw.WriteLine(linhaLog);
                }
                Debug.WriteLine("--> Evento salvo em arquivo local: " + linhaLog);
            }
            catch (Exception ex)
            {
                Debug.WriteLine("!!! ERRO AO SALVAR ARQUIVO LOCAL: " + ex.Message);
            }
        }
        ...
        private int ReceberVersaoBio6xx(Inner InnerAtual)
        {
            ...
            if (Ret == (int)Enumeradores.Retorno.RET_COMANDO_OK)
            {
                ...
                if (Ret == (int)Enumeradores.Retorno.RET_COMANDO_OK)
                {
                    InnerAtual.VersaoBio = "Versão bio " + Encoding.ASCII.GetString(VersaoBio);
                }
            }
            return Ret;
        }
        ...
        private void PASSO_ESTADO_CONECTAR(Inner InnerAtual)
        {
            ...
            try
            {
                ...
                {
                    //caso ele não consiga, tentará enviar dez vezes, se não conseguir volta para o passo Reconectar
                    if (InnerAtual.Tentativas >= 10)
                    {
                        InnerAtual.EstadoAtual = Enumeradores.EstadosInner.ESTADO_RECONECTAR;
                    }
            ...
            {                
                ...
                {   
                    ...
                    if (InnerAtual.Acionamento == (int)Enumeradores.Acionamento.Acionamento_Catraca_Entrada)
                    {
                        if (InnerAtual.CatInvertida)
                        {
                            EasyInner.DefinirEntradasMudancaOffLine((byte)(InnerAtual.Teclado ? Enumeradores.Opcao.SIM : Enumeradores.Opcao.NAO), 2, 0, 0);
                        }
                        else
                        {
                            EasyInner.DefinirEntradasMudancaOffLine((byte)(InnerAtual.Teclado ? Enumeradores.Opcao.SIM : Enumeradores.Opcao.NAO), 1, 0, 0);
                        }
                    }
                    else if (InnerAtual.Acionamento == (int)Enumeradores.Acionamento.Acionamento_Catraca_Saida)
                    {
                        if (InnerAtual.CatInvertida)
                        {
                            EasyInner.DefinirEntradasMudancaOffLine((byte)(InnerAtual.Teclado ? Enumeradores.Opcao.SIM : Enumeradores.Opcao.NAO), 1, 1, 0);
                        }
                        else
                        {
                            EasyInner.DefinirEntradasMudancaOffLine((byte)(InnerAtual.Teclado ? Enumeradores.Opcao.SIM : Enumeradores.Opcao.NAO), 2, 2, 0);
                        }
                    }
                    else
                    {
        ...
        private void PASSO_ESTADO_ENVIAR_MSG_OFF_CATRACA(Inner InnerAtual)
        {
            ...
            try
            {
                if (!InnerAtual.CatInvertida || (InnerAtual.Acionamento == (int)Enumeradores.Acionamento.Acionamento_Catraca_Entrada_E_Saida 
                                                                                     && InnerAtual.CatInvertida))
                {       
        ...
        private void PASSO_ESTADO_POLLING(Inner InnerAtual)
        {
            try
            {
                ...
                if (InnerAtual.TipoLeitor == (int)Enumeradores.TipoLeitor.QRCODE_LETRAS)
                {
                ...
                if (Ret == (int)Enumeradores.Retorno.RET_COMANDO_OK)
                {
                    ...
                    // DISPARA AS DUAS AÇÕES DE REGISTRO (API e LOCAL)
                    // Isso acontece *antes* do 'LiberarAcesso', mas não causa delay
                    // porque os métodos são "Async" (API) ou super rápidos (Arquivo).
                    EnviarEventoParaAPI(InnerAtual.BilheteOnline);
                    SalvarEventoLocalmente(InnerAtual.BilheteOnline);
            ...
            if (Ret == (int)Enumeradores.Retorno.RET_COMANDO_OK)
            {
                ...
                switch (Linha)
                {
                    ...
                    case 18:
                        ...
                        break;
                    case 19:
                        InnerAtual.LinhaInner = "Catraca 4";
                        InnerAtual.InnerNetAcesso = true;
                        break;
                    default:
                        InnerAtual.LinhaInner = "Nova linha";
                        InnerAtual.InnerNetAcesso = true;
                        break;
                }
